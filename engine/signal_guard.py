...
SEGMENT_RULES = {
    "NSE_OPTIONS": {"min_score":70,"min_rr":2.5,"min_net_profit":150,"max_sl_pct":30,"min_volume_ratio":1.3,"rsi_buy":(48,68),"rsi_sell":(32,52),"max_vix":22,"block_sideways":True},
    "MCX_OPTIONS": {"min_score":70,"min_rr":2.0,"min_net_profit":200,"max_sl_pct":35,"min_volume_ratio":1.2,"rsi_buy":(45,70),"rsi_sell":(30,55),"max_vix":25,"block_sideways":True},
    "NSE_EQUITY":  {"min_score":65,"min_rr":2.0,"min_net_profit":100,"max_sl_pct":2.0,"min_volume_ratio":1.5,"rsi_buy":(50,68),"rsi_sell":(32,50),"max_vix":20,"block_sideways":True},
}
...
def check_all(signal_data, market_data=None):
    ...
    else: failed.append("Score low "+str(score))
    rr=0
    if entry>0 and sl>0 and target>0:
        rr=round((target-entry)/(entry-sl),2) if dirn in ("BUY","CE") and (entry-sl)>0 else round((entry-target)/(sl-entry),2) if (sl-entry)>0 else 0
    if rr>=rules["min_rr"]: passed.append("RR ok "+str(rr))
    else: failed.append("RR low "+str(rr))
    if net<=0: passed.append("Net skip")
    elif net>=rules["min_net_profit"]: passed.append("Net ok")
    else: failed.append("Net low "+str(net))
    if entry>0 and sl>0:
        sl_pct=abs(entry-sl)/entry*100
        if sl_pct<=rules["max_sl_pct"]: passed.append("SL ok")
        else: failed.append("SL high "+str(round(sl_pct,1)))
    if vol>=rules["min_volume_ratio"]: passed.append("Vol ok")
    elif vol==1.0: passed.append("Vol skip")
    else: failed.append("Vol low "+str(vol))
    ...
    ok=len(crit_fail)==0
    sym=d.get("symbol",""); logger.info("[%s] %s %s p=%d f=%d",seg,sym,"OK" if ok else "BLOCK",len(passed),len(failed))
